...
loaded_image <- image_read("www/berlin_attention_check_Berlin_Hamburg.jpg")
    ...
    if (current_page == 1) {  # Initial instructions page
      ...
      tagList(
        div(id = "intro1", style = "text-align: center; margin: 0 auto; max-width: 800px; padding: 20px; background-color: #f4f6f9; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);",
            ...
            p("For each image, the Artificial intelligence model has predicted ", strong("the location"), ". We are providing you with this information.",
              style = "text-align: center;"),
      ...
      # Load Image
      image_path <- "www/berlin_attention_check_Berlin_Hamburg.jpg"
                ...
                div(class = "flex-container", 
                    ...
                    div(style = "background-color: #ffffff; border-radius: 8px; display: flex; flex: 1; flex-basis: 72%; padding: 15px; flex-direction: column; align-items: center;",  
                        div(style = "width: 100%; text-align: left; margin-bottom: 10px;",
                            p("This is an attention check. Please ", span(style = "color: #FFA500; font-weight: bold;", "highlight the yellow sign that says 'Berlin'"), " using the annotation tool. Before proceeding, ensure that you’ve marked the sign which says 'Berlin'.")
                        ),
                        # Display the image for annotation
          ...
          div(style = "display: flex; flex-direction: column; gap: 10px; width: 100%; margin: 0 auto;",  # Set gap to 0px to remove extra space
              ...
              div(class = "decision-container",
                  ...
                  div(class = "flex-container", 
                      ...
                      div(style = "background-color: #ffffff; border-radius: 8px; display: flex; flex: 1; flex-basis: 72%; padding: 15px; flex-direction: column; align-items: center;",  
                          div(style = "width: 100%; text-align: left; margin-bottom: 10px;",
                              p("Which parts of the image led you to your decision? Please ", span(style = "color: #FFA500; font-weight: bold;", "mark these key areas as precisely as possible"), " using the annotation tool. Before proceeding, ensure that you’ve marked all features you consider important.")
                          ),
    ...
    if (current_page == 10) {
      ...
      if (annotation_missing && city_not_selected) {
        ...
        if (selected_city_6() != "Berlin") {
          ...
          save_to_nextcloud(fail_text_path, fail_folder, paste0("control_", user_id(), ".txt"), username, password)
        ...
        img <- image_read("www/berlin_attention_check_Berlin_Hamburg.jpg")
        ...
        save_to_nextcloud(csv_temp_path, csv_attention_cloud_folder, paste0("control_", user_id(), "_", input_filename, "_", selected_class, "_", confidence_level, ".csv"), username, password)
        ...
        save_to_nextcloud(img_temp_path, img_attention_cloud_folder, paste0("control", "_",user_id(), "_", input_filename, "_", selected_class, "_", confidence_level, ".png"), username, password)
        ...
        csv_temp_path <- tempfile(fileext = ".csv")
        write.csv(save_coords, csv_temp_path, row.names = FALSE)
        save_to_nextcloud(csv_temp_path, csv_uebung_cloud_folder, paste0("control_", user_id(), "_", input_filename, ".csv"), username, password)
        
        
        # Save the annotated image temporarily and upload to Nextcloud
        img_temp_path <- tempfile(fileext = ".png")
        image_write(img, path = img_temp_path, format = "png")
        save_to_nextcloud(img_temp_path, img_uebung_cloud_folder, paste0("control", "_", user_id(), "_", input_filename, ".png"), username, password)
      ...
      if (annotation_missing && city_not_selected) {
        ...
        csv_temp_path <- tempfile(fileext = ".csv")
        write.csv(save_coords, csv_temp_path, row.names = FALSE)
        save_to_nextcloud(csv_temp_path, csv_cloud_folder, paste0("control", "_", user_id(), "_", input_filename, "_", selected_class,  "_",  confidence_level, ".csv"), username, password)
        ...
        img_temp_path <- tempfile(fileext = ".png")
        image_write(img, path = img_temp_path, format = "png")
        save_to_nextcloud(img_temp_path, img_cloud_folder, paste0("control", "_", user_id(), "_", input_filename, "_", selected_class, "_",  confidence_level, ".png"), username, password)
      ...
      if (is.null(input$q1) || length(input$q1) == 0 || 
        ...
        save_to_nextcloud(
          csv_temp_path, 
          question_folder, 
          paste0("control_", user_id(), "_questions.csv"), 
          username, 
          password
        )
